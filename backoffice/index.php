...
if (isset($_GET['msg'])) {
    ...
    $msg = match($_GET['msg']) {
        ...
        'imported_multi' => ['type' => 'success', 'text' => $n . ' fichiers JSON importés avec succès.'],
...
<?php endif; ?>
  ...
  <form method="get" class="row g-2 mb-3 align-items-end">
    ...
    <div class="col-auto d-flex gap-2 flex-wrap align-items-end">
      ...
      <?php
        $exportUrl = 'export_excel.php?' . http_build_query(array_filter([
            'q'    => $search,
            'dist' => $distrika,
        ]));
      ?>
      <a href="<?= htmlspecialchars($exportUrl) ?>" class="btn btn-success">
        <i class="bi bi-file-earmark-excel me-1"></i>Exporter Excel (.xls)
        <?php if ($search !== '' || $distrika !== ''): ?>
          <span class="badge bg-warning text-dark ms-1" title="Export selon les filtres actifs">filtrés</span>
        <?php else: ?>
          <span class="badge bg-light text-dark ms-1">tout</span>
        <?php endif; ?>
      </a>
    </div>
  </form>
